...
def count_hand(player_hand):
    ...
    for card, suit in hand:
        if card == 'A':
            aces += 1
            hand_value += 11
        elif card != 'A' and type(card) == str:
            hand_value += 10
        elif type(card) == int:
            hand_value += card

    # makes sure aces get counted properly
    if aces > 0 and hand_value > 21:
        hand_value = hand_value - (aces * 10)
...
def print_after_deal(all_playing, dealer):
    ...
    for player in deal_order:
        print(players_dict[player.id_tag], '\t', player.hand,
              '  ', player.hand_value)
    print(players_dict[dealer.id_tag], '\t', [player.hand[0]])
